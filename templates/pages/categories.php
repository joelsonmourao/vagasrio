...
<div class="entity-grid">
    <?php foreach ($categories as $category): ?>
        <article class="entity-card">
            <h2><a href="<?= e(base_url('/categoria/' . $category['slug'])) ?>"><?= e($category['name']) ?></a></h2>
            <p>Vagas ativas de <?= e($category['name']) ?> no estado do Rio de Janeiro.</p>
            <a class="btn btn-sm btn-outline" href="<?= e(base_url('/categoria/' . $category['slug'])) ?>">Explorar categoria</a>
        </article>
    <?php endforeach; ?>
</div>
