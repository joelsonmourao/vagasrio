    ...
    <div class="admin-shell admin-header-inner">
        ...
        <nav id="admin-nav" class="admin-nav">
            <?php foreach ([
                ...
                ?>
                <a class="admin-nav-link<?= $isActive ? ' is-active' : '' ?>" href="<?= e(base_url($href === '/admin' ? '/admin' : $href)) ?>"><?= e($label) ?></a>
            <?php endforeach; ?>
            <a class="admin-nav-link admin-nav-link-muted" href="<?= e(base_url('/')) ?>" target="_blank" rel="noopener">Ver site</a>
            <a class="admin-nav-link admin-nav-link-danger" href="<?= e(base_url('/admin/logout')) ?>">Sair</a>
        </nav>
        <?php else: ?>
        <a class="admin-brand" href="<?= e(base_url('/')) ?>">
            <span class="admin-brand-mark">RJ</span>
            <span class="admin-brand-text">
                <strong><?= e(config('site.name')) ?></strong>
